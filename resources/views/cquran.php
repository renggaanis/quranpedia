...
//     $data = database("SELECT `index`, surat_indonesia, surat_arab , arti, jumlah_ayat, tempat_turun FROM DaftarSurat");

//     echo '<table class="table table-striped table-hover table-sm table-bordered">';
//     echo '<thead><tr><th>No.</th><th>Surah</th><th>Arab</th><th>Arti</th><th>Jumlah Ayat</th></tr>Tempat turun</th><th></thead>';
//     foreach($data as $d){
//         echo '<tbody><tr><td>'.$d['index'].'</td><td><a href="quran?surat='.$d['index'].'&nama='.$d['surat_indonesia'].'">'.$d['surat_indonesia'].'</a></td><td>'.$d['surat_arab'].'</td></td></td><td>'.$d['arti'].'</td></td><td>'.$d['jumlah_ayat'].'</td><td>'.$d['tempat_turun'].'</td></td></tr></tbody>';
//     }
//     echo '</table>';
// }
...
function daftar(){
	mb_internal_encoding('UTF-8'); 
	$data = $this->db("SELECT `index`, surat_indonesia, surat_arab, arti, jumlah_ayat, tempat_turun FROM DaftarSurat");
    echo '<table class="table table-striped table-hover table-sm table-bordered">';
    echo '<thead><tr><th>No.</th><th>Surah</th><th>Arab</th><th>Arti</th><th>Jumlah Ayat</th><th>Tempat Turun</th></thead>';
...
function ayat($surat, $nama=''){	
	...
	$data = $this->db("SELECT A.AyahText as arabic, B.AyahText as indonesia, audio FROM quran A LEFT OUTER JOIN indo B ON A.id=B.id WHERE A.daftarsurat_id = $surat");
