...
function daftar(){
	...
	$data = $this->db("SELECT daftarbab.id, category.name, daftarbab.namasubbab,daftarbab.id, daftarbab.sub FROM category JOIN daftarbab ON category.id = daftarbab.category_id");
	 echo '<h2>Pemetaan Bab</h2>';
     echo '<table id="table-datatables"  class="table table-striped table-hover table-sm table-bordered">';
...
function ayat($sub, $nama=''){	
	...
	echo '<p><a href="http://localhost/webquranpedia/bab" class="fa fa-arrow-left"> Kembali ke Daftar Bab</a></p>';
	$data = $this->db("SELECT A.AyahText as arabic, B.AyahText as indonesia, A.VerseID as ayatarab, audio, A.daftarsurat_id as namasurat FROM quran A LEFT OUTER JOIN indo B ON A.ID=B.ID JOIN daftarbabsurat ON daftarbabsurat.daftarsurat_id = A.daftarsurat_id AND daftarbabsurat.verseID = A.VerseID LEFT OUTER JOIN daftarsurat ON daftarsurat.id = A.daftarsurat_id AND A.daftarsurat_id = daftarsurat.surat_indonesia WHERE daftarbabsurat.daftarbab_id = $sub");
	...
	foreach($data as $d){
        ...
        echo '<br/>';
        echo '<p>SURAT KE-'.$d['namasurat'].'</p>';
  		echo '<p class ="arabic" align="right" style="font-size: 30px">'. $str .' ('.$this->angka($d['ayatarab']).')'.'</p>';
		echo '<br><p class ="latin">'.$d['ayatarab']. '&nbsp' .$d['indonesia'] .'</p>';	
	...
	echo '<p align="center"><a href="http://localhost/webquranpedia/bab" > ***Kembali ke Daftar Bab ***</a></p>';
}
